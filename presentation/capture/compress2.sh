...
TARGET_MIB="${1:-46}"
RAW="film2-picture.mp4"
VOICE="../film2/vo/voice.wav"
OUT="freya-checks-${TARGET_MIB}mb.mp4"
...
DUR=$(python -c "
import re, pathlib
film = pathlib.Path('../film2')
js = (pathlib.Path('../film') / 'film.js').read_text(encoding='utf-8')
...
if [ -f work/x264b-0.log ] && [ work/x264b-0.log -nt "$RAW" ]; then
    echo "pass 1 already done, reusing work/x264b-0.log"
else
    echo "pass 1 of 2"
    ffmpeg -hide_banner -loglevel error -y -ss "$SS" -i "$RAW" -t "$DUR"         "${ENCODE[@]}" -pass 1 -passlogfile work/x264b -an -f null -
fi

echo "pass 2 of 2"
ffmpeg -hide_banner -loglevel error -y -ss "$SS" -i "$RAW" -i "$VOICE" -t "$DUR"     -map 0:v:0 -map 1:a:0 "${ENCODE[@]}" -pass 2 -passlogfile work/x264b     -c:a aac -b:a "${AUDIO_KBPS}k" -ac 1 -shortest -movflags +faststart "$OUT"
